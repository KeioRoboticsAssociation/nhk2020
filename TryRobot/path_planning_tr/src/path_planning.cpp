...
void Path::set_vel(){
	...
	for (int p = 2; p <= pnum - 1; p++) {
		...
		float v;
		if (k < 1e-8) v = max_vel;
		else v = min(max_vel, sqrt(max_accel / (k * 1000)));
		point[p][4] = v;
	}
	...
	for (int _ = 1; _ <= pnum; _++) {
		...
		if (index - 1 >= 1) {
			...
			if (abs(accel) > max_accel) {
				if (accel > 0) point[index - 1][4] = sqrt(point[index][4] * point[index][4] + 2 * point[index - 1][5] / 1000.0f * max_accel);
				else point[index - 1][4] = sqrt(point[index][4] * point[index][4] - 2 * point[index - 1][5] / 1000.0f * max_accel);
			}
		}
		...
		if (index + 1 <= pnum) {
			...
			if (abs(accel) > max_accel) {
				if (accel > 0) point[index + 1][4] = sqrt(point[index][4] * point[index][4] + 2 * point[index][5] / 1000.0f * max_accel);
				else point[index + 1][4] = sqrt(point[index][4] * point[index][4] - 2 * point[index][5] / 1000.0f * max_accel);
			}
		}
...
Matrix Path::path_func(float t) {
	...
	for (int xy = 1; xy <= 2; xy++) {
		ans[xy][1] = pow(1 - tt, 3) * point[Q][xy]
			+ 3 * pow(1 - tt, 2) * tt * control_point[2 * Q - 1][xy]
			+ 3 * (1 - tt) * pow(tt, 2) * control_point[2 * Q][xy]
			+ pow(tt, 3) * point[Q + 1][xy];
	}
...
Matrix Path::solve_equations(Matrix M_, Matrix y_) {
	...
	for (int i = 1; i <= size; i++) {
		...
		if (abs(Mtemp) < 1e-8) {	// non-invertible
...
float Path::bezier_length(float px[4], float py[4], float converge) {
	...
	float newpx2[4] = { (px[0] + 3 * px[1] + 3 * px[2] + px[3]) / 8,
		(px[1] + 2 * px[2] + px[3]) / 4,
		(px[2] + px[3]) / 2,
		px[3],};
	float newpy2[4] = { (py[0] + 3 * py[1] + 3 * py[2] + py[3]) / 8,
		(py[1] + 2 * py[2] + py[3]) / 4,
		(py[2] + py[3]) / 2,
		py[3], };
