...
void wheel_control(float theta_body, float vx, float vy, float omega)
{
  if (flag == 2)  // reset
  {
    for (int i = 0; i < 4;i++){
  ...
  wheelpos2[0][0] = BODY_WIDTH / sqrt(2.0f) * cos(theta_body + omega + PI / 4.0f);
  wheelpos2[0][1] = BODY_WIDTH / sqrt(2.0f) * sin(theta_body + omega + PI / 4.0f);
